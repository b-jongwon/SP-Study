...
// Return variable value (check local, then global, then environment)
...
// Set or update a global variable and also set it in the environment
...
// Replace all $VAR tokens in the input line with their actual values
...
// Tokenize command line and expand variables
...
// Execute an external command, supporting < / > redirection
int execute_external_command(char **args) {
    ...
    if (pid < 0) {
        ...
        // Parse arguments and handle redirection symbols
...
// Forward declaration needed for recursion in handle_if_block
void execute_command(char **args);

// Handle if-then-fi block
// Read lines until 'fi' is found
void handle_if_block(char *if_line, FILE *input) {
    ...
    // PDF 로직에 맞춰 조건 라인 파싱 (if_line + 3 부터 복사)
    if (strlen(if_line) > 3) {
    ...
    // Expect the next line to be 'then'
    char line[MAX_LINE];
    ...
    // Evaluate the condition and execute the block if successful
    char *args[MAX_ARGS];
    parse_command(cond_line, args);
    int cond_result = execute_external_command(args);

    if (cond_result == 0) { // Success (exit code 0)
        for (int i = 0; i < count; i++) {
             // 재귀적 실행을 위해 process_line 대신 parse -> execute 흐름 사용
             // (PDF 예시 코드 흐름을 따름)
...
void execute_command(char **args) {
    ...
    // VAR=val syntax: assign local variable
...
// Process a single line (used in both interactive and script mode)
